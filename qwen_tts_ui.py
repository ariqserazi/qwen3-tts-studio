...
# Proxy bypass for local Gradio + local services
os.environ["no_proxy"] = "localhost,127.0.0.1"
os.environ["NO_PROXY"] = "localhost,127.0.0.1"

# macOS + Apple Silicon stability helpers
if platform.system() == "Darwin":
    # Allow fallback to CPU for ops not supported on MPS instead of crashing
    os.environ.setdefault("PYTORCH_ENABLE_MPS_FALLBACK", "1")
    # Avoid tokenizer parallelism warnings and occasional deadlocks on macOS
...
def _mac_safe_torch_cleanup():
    """
    macOS oriented cleanup to reduce MPS memory pressure and avoid crashes.
    Keeps your existing _mps_cleanup but adds a safe torch.mps cache clear.
    """
    try:
        gc.collect()
    except Exception:
...
def enhance_punctuation_for_tts(text: str) -> str:
    """
    Convert punctuation into speech cues so the TTS reacts more naturally.
    """
...
def format_user_error(error: Exception) -> str:
    error_messages = {
        ...
        "MPS backend out of memory": "Not enough Apple GPU memory. Try reducing text length, using fewer segments, or switching to a smaller model.",
        "not implemented for 'mps'": "This operation is not supported on Apple GPU. Try again, the app will fall back to CPU for unsupported ops.",
...
PODCASTS_DIR = BASE_OUTPUT_DIR / "podcasts"
                ...
                if meta_path.exists() and audio_path.exists():
                    with open(meta_path) as f:
                        ...
                        if outline_path.exists():
                            with open(outline_path) as f2:
                                meta["outline"] = json.load(f2)
                        if transcript_path.exists():
                            with open(transcript_path) as f2:
                                meta["transcript"] = json.load(f2)
...
def clone_voice_multi(
    ...
    test_text = enhance_punctuation_for_tts(test_text)
    if not audio_files:
    ...
    try:
        ...
        samples_meta = [
            {
                "path": str(s.path),
                "duration": float(s.duration) if s.duration is not None else 0.0,
                "transcript": str(s.transcript or ""),
                "snr_estimate": float(s.snr_estimate) if s.snr_estimate is not None else None,
        ...
        samples_meta_json = json.dumps(_json_safe(samples_meta), ensure_ascii=False)
...
def generate_with_saved_voice(
    ...
    try:
        ...
        plan = _process_text_with_pauses(text)
        speech_texts = [p["text"] for p in plan if p["type"] == "speech"]

        all_wav_chunks = []
        sr = None

        for seg_text in speech_texts:
            wavs, sr = model.generate_voice_clone(
                text=seg_text,
                language=language,
                voice_clone_prompt=voice_clone_prompt,
                ...
                top_p=top_p,
                repetition_penalty=repetition_penalty,
                max_new_tokens=estimate_max_tokens(seg_text),
                subtalker_temperature=sub_temp,
                subtalker_top_k=int(sub_top_k),
                subtalker_top_p=sub_top_p,
            )
            all_wav_chunks.append(wavs[0])
        ...
        for item in plan:
            if item["type"] == "speech":
                final_stream.append(all_wav_chunks[speech_idx])
                speech_idx += 1
            else:
                silence_samples = int(sr * item["duration"])
                final_stream.append(np.zeros(silence_samples, dtype=np.float32))

        combined_wav = np.concatenate(final_stream, axis=0)

        gen_time = time.time() - start_time

        progress(0.9, desc=f"Saving audio ({gen_time:.1f}s)...")
        with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as f:
            sf.write(f.name, combined_wav, sr)
            history_path = save_to_history(
                f.name,
                text,
...
# Keep your full custom_css exactly as you had it.
# I am not changing its contents for macOS, just keeping your file as one piece.
...
def _generate_persona_voice_preview(voice_id: str, voice_type: str) -> str | None:
    try:
        from ui.voice_cards import generate_preview as _gen

        return _gen(voice_id, voice_type)
    except Exception as e:
...
# UI
# --------------------------------------------------------------------------------------
# Paste this whole Blocks section into your main file, replacing your existing
# with gr.Blocks(...) as demo: block all the way down to the end of the UI wiring.
def save_output_dir(p: Path) -> None:
    payload = {"output_dir": str(p)}
...
def load_output_dir() -> Path:
    if OUTPUT_DIR_FILE.exists():
        try:
            with open(OUTPUT_DIR_FILE) as f:
                data = json.load(f)
            raw = str(data.get("output_dir", "")).strip()
            if raw:
...
def set_base_dirs(base: Path):
    global BASE_OUTPUT_DIR, HISTORY_DIR, PODCASTS_DIR

    BASE_OUTPUT_DIR = Path(base).resolve()
    HISTORY_DIR = (BASE_OUTPUT_DIR / "generation_history").resolve()
    PODCASTS_DIR = (BASE_OUTPUT_DIR / "podcasts").resolve()

    HISTORY_DIR.mkdir(parents=True, exist_ok=True)
    PODCASTS_DIR.mkdir(parents=True, exist_ok=True)


set_base_dirs(load_output_dir())


def open_folder_in_finder(folder_path: str):
...
def apply_output_dir(new_dir: str):
    if not new_dir or not str(new_dir).strip():
        return gr.update(), "No folder selected"
    ...
    try:
        ...
        save_output_dir(p)
        set_base_dirs(p)
        return gr.update(value=str(p)), f"Using: {p}"
    except Exception as e:
        return gr.update(), f"Error: {format_user_error(e)}"

with gr.Blocks(title="Qwen3-TTS Studio") as demo:
    gr.HTML("<div style='font-weight:600;margin:0.25rem 0;'>Output folder for new generations</div>")
    with gr.Row():
        output_dir_text = gr.Textbox(value=str(BASE_OUTPUT_DIR), label="Output folder path", scale=4)
        ...
        with gr.Column(scale=5):
            with gr.Tabs() as tabs:
                ...
                with gr.TabItem("Podcast", id="podcast"):
                    ...
                    podcast_session_state = gr.State(None)
                    ...
                    with gr.Row():
                        with gr.Column(scale=1):
                            ...
                            _initial_voice_choices = _get_podcast_voice_choices()
                            for i in range(4):
                                with gr.Row():
                                    ...
                                    podcast_speaker_slots.append((slot_role, slot_voice))
...
if __name__ == "__main__":
    ...
    server_port=7860,
    css=custom_css,
    allowed_paths=[str(BASE_OUTPUT_DIR)],
)